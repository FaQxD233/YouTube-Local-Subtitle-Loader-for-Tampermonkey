// ==UserScript==
// @name         YouTube Local Subtitles - Dev
// @name:en      YouTube Local Subtitles - Dev
// @name:zh-CN   YouTube 本地字幕 - 开发版
// @name:zh-TW   YouTube 本地字幕 - 開發版
// @namespace    https://github.com/FaQxD233/YouTube-Local-Subtitle-Loader-for-Tampermonkey/dev
// @version      1.8-dev
...
(function () {
  ...
  const STATE_KEY = '__ytLocalNativeCcDevState';
  ...
  const state = window[STATE_KEY] || {
    ...
    initTimer: null,
    pageObserverAttached: false,
    deferredTrackInjected: false,
  ...
  function loadLocalSubtitles(text, filename) {
    ...
    forceRefreshCaptionsModule();
    refreshNativeCaptionTracklist();
    setTimeout(refreshNativeCaptionTracklist, 250);
    setTimeout(refreshNativeCaptionTracklist, 800);
    updateMenuStatus();
  ...
  function forceRefreshCaptionsModule() {
    const player = getPlayer();
    if (!player) return;

    try { player.unloadModule && player.unloadModule('captions'); } catch (err) {}
    setTimeout(() => {
      try { player.loadModule && player.loadModule('captions'); } catch (err) {}
      refreshNativeCaptionTracklist();
    }, 50);
  ...
  function setupSettingsObserver() {
    ...
    const observer = new MutationObserver(() => injectSettingsMenuItems(settingsMenu));
    observer.observe(settingsMenu, { childList: true, subtree: true });
    settingsMenu._tmLocalNativeCcObserverAttached = true;
    injectSettingsMenuItems(settingsMenu);
  }

  function injectSettingsMenuItems(settingsMenu) {
    injectUploadMenuItem(settingsMenu);
  }

  function setupPageObserver() {
    if (state.pageObserverAttached || !document.documentElement) return;
    ...
    state.pageObserverAttached = true;
  }

  function setupPlaybackObserver() {
    if (state.playbackObserverAttached) return;
    state.playbackObserverAttached = true;

    const timer = setInterval(() => {
      if (state.deferredTrackInjected) {
        clearInterval(timer);
        return;
      }
      ...
      if (player.classList && player.classList.contains('ad-showing')) return;
      if (!Number.isFinite(video.currentTime) || video.currentTime < 2) return;

      state.deferredTrackInjected = true;
      clearInterval(timer);
      injectPlaceholderTrackAfterPlayback();
    }, 1000);
  }

  function injectPlaceholderTrackAfterPlayback() {
    ensureLocalTrack();
    patchInitialPlayerResponseSetter();
    injectKnownPlayerResponses();
    forceRefreshCaptionsModule();
    refreshNativeCaptionTracklist();
    updateMenuStatus();
    debugLog('placeholder track injected after playback');
  }
  ...
  window.addEventListener('yt-navigate-start', () => {
    ...
    state.filename = '';
    state.track = null;
    state.deferredTrackInjected = false;
    state.playbackObserverAttached = false;
